...
Collision Snake::Move(const Direction newDirection, const Vector2 &applePosition)
{
...
{
	const Direction dir =
		this->hasDirection() && !isPerpDirections(direction, _headDirection) ? _headDirection : direction;
...
{
	return _headDirection != Direction::None;
}
...
Collision Snake::detectCollision(const Vector2 &applePosition) const
{
	...
	if (Vector2Equals(headPosition, applePosition))
		return Collision::Apple;

	if (!utils::inBounds(headPosition))
		return Collision::Wall;

	const auto collidesWithHead = [&headPosition](const auto &pos) { return Vector2Equals(pos, headPosition); };
	if (this->hasDirection() && std::any_of(std::next(_body.rbegin()), _body.rend(), collidesWithHead))
...
{
	return std::ranges::any_of(_body, [&position](const auto &bodyPos) { return Vector2Equals(position, bodyPos); });
}
	...
	for (std::size_t idx = 0; const auto &[x, y] : _body)
	{
		...
		const Color color = idx == 0				  ? specs::snakeHeadColor
							: idx + 1 == _body.size() ? specs::snakeTailColor
													  : specs::snakeColor;
		DrawRectangleRounded(snakeBodyPart, bodyPartRoundness, bodyPartSegments, color);
		++idx;
	}
}

std::size_t Snake::SnakeLength() const
{
	return _body.size();
}
